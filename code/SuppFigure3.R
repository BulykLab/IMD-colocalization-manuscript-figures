...
## ATAC & RNA vs. ATAC vs. RNA

## Supp Figure - S3
mesc_proportion <- read.table('~/Library/CloudStorage/GoogleDrive-rjeong@g.harvard.edu/My Drive/Manuscript/IMD_colocalization/draft/data/Fig2/ATAC_RNA_joint.mesc.results.proportion.070423.txt', header = T)
...
#   scale_fill_manual(labels = c(expression("caQTL "*textstyle(intersect())*" eQTL"),"Just caQTL", "Just eQTL"), values = c("#9F56C8","#89B2F5", "#FF6666")) +


ggsave('~/Library/CloudStorage/GoogleDrive-rjeong@g.harvard.edu/My Drive/Manuscript/IMD_colocalization/draft/figures/FigureS3.pdf', 
       plot = p_S3, 
...
go_enrichment <- read.table('~/Library/CloudStorage/GoogleDrive-rjeong@g.harvard.edu/My Drive/Manuscript/IMD_colocalization/draft/data/Fig2/GO_eQTL_enrichment_nearby.txt', header = T, sep='\t')
