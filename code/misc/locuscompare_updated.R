...
assign_color2=function(rsid,snp,ld){
    ...
    ld = ld[ld$SNP_A==snp,]
    ld$color = as.character(cut(ld$R2,breaks=c(0,0.2,0.4,0.6,0.8,1), labels=c("lightgray", "lightskyblue", "green", "orange", "red"), include.lowest=TRUE))

    color = data.frame(rsid, stringsAsFactors = FALSE)
    color = merge(color, ld[, c('SNP_B', 'color')], by.x = 'rsid', by.y = 'SNP_B', all.x = TRUE)
    color[is.na(color$color),'color'] = 'lightgray'
...
assign_fill2=function(rsid,snp,ld){
...
make_locuszoom2=function(metal,title,chr,color,shape,size,ylab_linebreak=FALSE, range, legend=FALSE, legend_position = c('bottomright','topright','topleft')){
    ...
    p = ggplot(metal[(metal$pos >= range[1] & metal$pos <= range[2]),],aes(x=pos,logp))+
        geom_point(aes(fill=rsid,size=rsid,shape=rsid),alpha=0.5)+
...
make_locuszoom2_nolabel=function(metal,title,chr,color,shape,size,ylab_linebreak=FALSE, range, legend=FALSE, legend_position = c('bottomright','topright','topleft')){
